...
flowfile<-read.FCS("",alter.names=TRUE) #Choose your example file, the alter.names changes the headers into on R safe format (no spaces)
flowfile
params<-c("FSC", "SSC") #Insert you parameters of interest here, add as many as you like
...
for (fileName in files) {
  ...
  if (keyword(flowfile, "INDEXSORTPOSITIONS") %in% keyword(flowfile) ==TRUE){
    ...
    #removes excess columns, remove if you want it all
